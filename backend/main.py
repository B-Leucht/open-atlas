...
def get_district(district_id):
    ...
    try:
        ...
        return jsonify({"success": True, "data": district.to_dict()})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
                ...
                datasets.append(
                    {
                        ...
                        "title": ds.title,
                        "feature_count": len(
                            [f for f in all_features if f.dataset_id == ds_id]
                        ),
                    }
                )
...
def aggregate_by_district():
    ...
    try:
        ...
        if not dataset_id:
            return jsonify(
                {"success": False, "error": "dataset parameter required"}
            ), 400
...
def get_index_preset_details(preset_id):
    ...
    try:
        ...
        return jsonify({"success": True, "preset": details})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_dataset_features(dataset_id):
    ...
    try:
        ...
        geojson = db.get_features_geojson(
            dataset_id=dataset_id, district_id=district_id, limit=limit
        )
        ...
        if not geojson.get("features") and not dataset_id.startswith("munich_"):
            geojson = db.get_features_geojson(
                dataset_id=f"munich_{dataset_id}", district_id=district_id, limit=limit
            )
...
def sync_districts():
    ...
    try:
        ...
        count = service.ingest_districts(
            MUNICH_DATA_SOURCE if source_id == "munich" else None
        )

        return jsonify(
            {"success": True, "message": f"Synced {count} districts", "count": count}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_data_sources():
    ...
    try:
        ...
        return jsonify({"success": True, "sources": [s.to_dict() for s in sources]})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
    ...
    return jsonify(
        {
            ...
            "version": "7.0.0",
            "endpoints": {
                "General": {"GET /api/health": "Health check"},
                "Districts": {
